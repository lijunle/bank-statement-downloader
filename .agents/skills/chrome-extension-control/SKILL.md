...
debugging. Bind CDP to loopback. Have the shell choose an unused random dynamic port and pass that
specific port to Chrome. Do not use `--remote-debugging-port=0`; Chrome's automatic port
allocation behaves differently from an explicitly assigned port.

Chrome's process-singleton mechanism prevents two browser instances from owning the same profile.
If the profile is already open, recover the explicitly assigned port from that browser's root
process and verify its endpoint instead of launching another instance.
...
- the root browser process uses the expected profile and explicit debugging port;
...
The reference below reuses the browser that owns the profile or has PowerShell choose an unused
random dynamic port before launching Chrome, then connects a scoped CLI daemon. If `status`
already reports the same browser URL and the extension category, reuse that daemon and omit
`--sessionId` from later commands.

```powershell
$chrome = "C:\Program Files\Google\Chrome\Application\chrome.exe"
$profile = "$HOME\.cache\Google\Chrome\bank-sync"
$chromeDevtools = ".\node_modules\.bin\chrome-devtools.cmd"
$profilePattern = '(?:^|\s)"?--user-data-dir="?' +
...
if ($profileOwner) {
  $portMatch = [regex]::Match(
    $profileOwner.CommandLine,
    '(?:^|\s)"?--remote-debugging-port=(\d+)"?(?=\s|$)'
  )
  if (-not $portMatch.Success) {
    throw "The bank-sync profile is open without an explicit DevTools port."
  }
...
do {
  try {
    $version = Invoke-RestMethod "$browserUrl/json/version" -TimeoutSec 2
  } catch {
    $version = $null
...
if (-not $version) {
  throw "Chrome did not publish a working DevTools endpoint on port $port."
}
...
$profileOwner = Get-CimInstance Win32_Process -Filter "Name = 'chrome.exe'" |
  Where-Object {
    ...
    $_.CommandLine -match "(?:^|\s)`"?--remote-debugging-port=$port`"?(?=\s|$)"
  } |
  Select-Object -First 1
if (-not $profileOwner) {
  throw "The DevTools endpoint does not belong to the expected Chrome profile."
}

$browserProcess = Get-Process -Id $profileOwner.ProcessId
$browserProcess |
  Select-Object Id, MainWindowTitle, MainWindowHandle
if (-not $browserProcess.MainWindowHandle) {
  throw "The expected Chrome process does not own a visible window."
}
